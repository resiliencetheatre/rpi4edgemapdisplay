                    ...
                    janus.attach(
                        {
                            ...
                            onlocaltrack: function(track, on) {
                                ...
                                if($('#mute').length === 0) {
                                    ...
                                    // $('#videolocal').append('<button class="button-on-video" id="mute">Mute</button>');
                                    
                                    $('#videolocalbuttons').append('<button class="button-video" id="mute">Mute</button>');
                                    
                                    $('#mute').click(toggleMute);
                                    
                                    // Add an 'unpublish' button
                                    // $('#videolocal').append('<button class="btn btn-warning btn-xs" id="unpublish" style="position: absolute; bottom: 0px; right: 0px; margin: 15px;">Unpublish</button>');
                                    // $('#videolocal').append('<button class="button-video" id="unpublish" style="position: absolute; top: 0px; right: 0px; margin: 5px;">Unpublish</button>');
                                    // myvideo vs videolocal
                                    // AKU
                                    $('#videolocalbuttons').append('<button class="button-video" id="unpublish" >Unpublish</button>');
                            ...
                            oncleanup: function() {
                                ...
                                // $('#videolocal').html('<button class="button-video" id="publish" class="btn btn-primary">Publish</button>'); 
                                // $('#videolocal').html('<button class="map-videoroom-ctrl-buttons" id="publish"  >Publish</button>');
                                $('#videolocalbuttons').html('<button class="button-video" id="publish">Publish</button>'); // AKU 2
                                ...
                                $('#videolocal').html('');
                                console.log("** oncleanup completed **")
                            }
                        }
                        );
                },
	...
	janus.attach(
		{
			...
			onremotetrack: function(track, mid, on) {
				...
				if(track.kind === "audio") {
                    ...
                    // Keep it simple, no BW info here
                    if ( 0 ) 
                    {
                        $('#videoremote'+remoteFeed.rfindex).append(
                            // '<span class="label label-primary hide map-videoroom-metadata" id="curres'+remoteFeed.rfindex+'" style="position: absolute; bottom: 0px; left: 0px; margin: 10px;"></span>' +
                            '<span class="hide map-videoroom-metadata" id="curres'+remoteFeed.rfindex+'" style="position: absolute; bottom: 5px; left: 0px; margin: 10px;"></span>' +
                            // '<span class="label label-info hide map-videoroom-metadata" id="curbitrate'+remoteFeed.rfindex+'" style="position: absolute; bottom: 0px; right: 0px; margin: 5px;"></span>');
                            '<span class="hide map-videoroom-metadata" id="curbitrate'+remoteFeed.rfindex+'" style="position: absolute; bottom: 5px; right: 0px; margin: 10px;"></span>');
                        // Janus.attachMediaStream($('#remotevideo' + remoteFeed.rfindex + '-' + mid).get(0), stream);
